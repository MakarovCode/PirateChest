...
module Serket
    ...
    class FieldDecrypter
        attr_accessor :field_delimiter, :private_key_filepath, :symmetric_algorithm, :encoding, :dummy_private_key_path, :dummy_password

        def initialize(options = {})
            options ||= {}

            @private_key_filepath   = Serket.configuration.private_key_path
            @dummy_private_key_path  = Serket.configuration.dummy_private_key_path
        ...
        def decrypt(field, key_path, pass)
            return if field !~ /\S/
            iv, encrypted_aes_key, encrypted_text = parse(field)
            private_key = OpenSSL::PKey::RSA.new(File.read(key_path), pass)
            #puts "=======>#{parse(field)} ===> #{encrypted_text} ==> #{encrypted_aes_key} => #{iv}"
